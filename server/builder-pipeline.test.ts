...
describe("Builder Tool Definitions", () => {
  ...
  it("should have exactly 43 tools total", () => {
    expect(TITAN_TOOLS.length).toBe(44);
  });
...
describe("Multi-file Validation", () => {
  it("should reject when exceeding max files per operation (>5)", () => {
    const mods: ModificationRequest[] = Array.from({ length: 10 }, (_, i) => ({
  ...
  it("should accept up to 5 valid modifications", () => {
    const mods: ModificationRequest[] = Array.from({ length: 5 }, (_, i) => ({
      filePath: `server/test-file-${i}.ts`,
      action: "create" as const,
  ...
  it("should reject modifications with dangerous patterns", () => {
    const mods: ModificationRequest[] = [
      {
