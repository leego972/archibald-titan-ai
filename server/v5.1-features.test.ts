...
describe("Self-Improvement Engine", () => {
  ...
  describe("Validation", () => {
    ...
    it("should reject oversized content (>500KB)", () => {
    ...
    it("should allow process.exit and eval (loosened restrictions)", () => {
      const mods1: ModificationRequest[] = [
        { filePath: "server/test-file.ts", action: "modify", content: "process.exit(1);" },
      ];
      expect(validateModifications(mods1).valid).toBe(true);

      const mods2: ModificationRequest[] = [
        { filePath: "server/test-file.ts", action: "modify", content: 'eval("alert(1)")' },
      ];
      expect(validateModifications(mods2).valid).toBe(true);
    });

    it("should reject truly destructive patterns like rm -rf /", () => {
      const mods: ModificationRequest[] = [
        { filePath: "server/test-file.ts", action: "modify", content: 'rm -rf /' },
      ];
      const result = validateModifications(mods);
      expect(result.valid).toBe(false);
