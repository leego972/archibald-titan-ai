...
// Task economics data — shows what credits actually buy
const TASK_EXAMPLES = [
  { task: "Chat message to Titan", credits: 1, icon: "💬", category: "Chat" },
  { task: "Builder: landing page", credits: 5, icon: "🏗️", category: "Builder" },
  { task: "Builder: full project scaffold", credits: 10, icon: "🚀", category: "Builder" },
  { task: "Credential fetch (1 provider)", credits: 1, icon: "🔑", category: "Fetcher" },
  { task: "Leak scan (1 domain)", credits: 3, icon: "🔍", category: "Security" },
  { task: "Website clone", credits: 15, icon: "🌐", category: "Builder" },
  { task: "Business plan generation", credits: 8, icon: "📋", category: "Builder" },
  { task: "SEO keyword research", credits: 4, icon: "📈", category: "SEO" },
];
  ...
  return (
    <div className="max-w-4xl mx-auto mt-16 mb-8">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-white mb-2">What do credits actually buy?</h2>
        <p className="text-white/40 text-sm max-w-xl mx-auto">Credits are the universal currency for all AI actions. Here are real examples of what each action costs.</p>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {TASK_EXAMPLES.map((t) => (
          <div key={t.task} className="p-4 rounded-xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-all">
            <div className="text-2xl mb-2">{t.icon}</div>
            <div className="text-xs text-white/50 leading-snug mb-2">{t.task}</div>
            <div className="flex items-center justify-between">
              <span className="text-sm font-bold text-white">{t.credits} cr</span>
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/5 text-white/30">{t.category}</span>
            </div>
          </div>
        ))}
      </div>
      <p className="text-center text-xs text-white/25 mt-4">Credits refresh monthly. Unused credits do not roll over. Top-up packs available in your dashboard.</p>
    </div>
  );
}
...
const pricingFaqs = [
  ...
  {
    q: "Is Titan Builder available on the free plan?",
    a: "No. Titan Builder is available on Pro, Enterprise, Cyber, Cyber+, and Titan plans. Free plan users have full access to Titan Chat but Builder — which produces structured, exportable outputs like landing pages, MVPs, scripts, and documents — requires a paid plan.",
  },
  {
    q: "Can I use Titan Builder for client work?",
    a: "Yes. All outputs generated by Titan Builder are yours to use commercially. There are no licensing restrictions on code, documents, or assets produced by the Builder.",
  },
];
