...
const DestroMask = ({ volume = 0, emotion = 'neutral' }: DestroMaskProps) => {
  ...
  useFrame((state) => {
    ...
    // Default values
    let eyeColor = '#00ffff'; // Default blue
    let glowIntensity = 2 + volume * 15;
    let jawScaleX = 0.6;
    let distort = 0.2;
    let speed = 2;

    // Nuanced Emotional Logic
    switch (emotion) {
      case 'thinking':
        eyeColor = '#00ccff';
      ...
      case 'amused':
        ...
        break;
      case 'friendly_stern':
        eyeColor = '#00eebb'; // Warmer cyan
        glowIntensity = 2.5 + volume * 10;
        jawScaleX = 0.65; // Subtle smirk
        break;
      case 'laughing':
        eyeColor = '#00ffff';
