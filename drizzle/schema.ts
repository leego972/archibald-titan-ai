...
/**
 * Director profiles — links a user to a Virelle Studios director account.
 */
export const virelleDirectors = mysqlTable("virelle_directors", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().unique(),
  studioName: varchar("studioName", { length: 255 }).notNull().default("My Studio"),
  bio: text("bio"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
export type VirelleDirector = typeof virelleDirectors.$inferSelect;
export type InsertVirelleDirector = typeof virelleDirectors.$inferInsert;

/**
 * Scenes — the core content unit for Virelle Studios.
 * Can be internal (AI-generated) or external (user-uploaded/linked).
 */
export const virelleScenes = mysqlTable("virelle_scenes", {
  id: int("id").autoincrement().primaryKey(),
  ...
  externalSource: varchar("externalSource", { length: 128 }),
  content: json("content").$type<Record<string, unknown>>(),
  notes: text("notes"),
  status: mysqlEnum("status", ["draft", "published", "archived"]).default("draft").notNull(),
  copyrightAcknowledged: boolean("copyrightAcknowledged").default(false).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
export type VirelleScene = typeof virelleScenes.$inferSelect;
export type InsertVirelleScene = typeof virelleScenes.$inferInsert;
