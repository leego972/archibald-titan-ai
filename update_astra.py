...
# Update execAstraApi to use titan exec if it's the titan server
old_exec_api = """async function execAstraApi(
  ssh: SSHConfig,
  path: string,
  method: "GET" | "POST",
  body?: object,
...
new_exec_api = """async function execAstraApi(
  ssh: SSHConfig,
  path: string,
  method: "GET" | "POST",
  body?: object,
...
# Update all calls to execAstraApi to pass ctx.user.id
content = re.sub(r'execAstraApi\(([^,]+),\s*([^,]+),\s*([^,]+)\)', r'execAstraApi(\1, \2, \3, undefined, 60000, ctx.user.id)', content)
content = re.sub(r'execAstraApi\(([^,]+),\s*([^,]+),\s*([^,]+),\s*([^,]+)\)', r'execAstraApi(\1, \2, \3, \4, 60000, ctx.user.id)', content)
content = re.sub(r'execAstraApi\(([^,]+),\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*([^)]+)\)', r'execAstraApi(\1, \2, \3, \4, \5, ctx.user.id)', content)
